...
function [color_score__,edge_score__,spatio_score__,score__] = lg_model(movie_name, fps,sigma_val)
...
%     end
   ...
   spatio_lg__=zeros(1,nFrames-init_frame+1);
   color_lg__=zeros(1,nFrames-init_frame+1);
   
   for k = init_frame : nFrames
%        color_lg__(k)=sigmoid(half_convolve(color_hist_values(k,:),k,gauss_filter));
%        spatio_lg__(k)=sigmoid(half_convolve(spatio_hist_values(k,:),k,gauss_filter));
        color_lg__(k)=half_convolve(color_hist_values(k,:),k,gauss_filter);
        spatio_lg__(k)=half_convolve(spatio_hist_values(k,:),k,gauss_filter);

   end


    [color_score__,edge_score__,spatio_score__,score__] = video_score(color_lg__,edge_hist_values, spatio_lg__);
toc;
